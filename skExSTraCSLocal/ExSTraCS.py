...
class ExSTraCS(BaseEstimator,ClassifierMixin):
    def __init__(self,learning_iterations=100000,N=1000,nu=1,chi=0.8,mu=0.04,theta_GA=25,theta_del=20,theta_sub=20,
                 ...
                 track_accuracy_while_fit=True,random_state=None,log_dir="", log_popfile_name="popfile.txt", log_trainingfile_name="ab.txt", 
                 enable_earlystopping=False, early_stopping_count=0, min_acc=0.001):
        '''
        ...
        log_popfile_path = log_dir + log_popfile_name
        self.log_popfile_name = open(log_popfile_path, 'w', newline='')
        self.log_popfile_name.write("IterationNo RuleID Condition Action Accuracy Fitness Numerosity CorrectCount\n")
        ...
        self.enable_earlystopping = enable_earlystopping
    ...
    def fit(self, X, y):
        ...
        for _ in tqdm(range(self.iterationCount, self.learning_iterations), desc="Model Training Progress"):
        #while self.iterationCount < self.learning_iterations:
            ...
            accuracy = sum(self.trackingAccuracy)/len(self.trackingAccuracy) if len(self.trackingAccuracy) != 0 else 0
            ...
            #Stop model training if not learning anymore depending on the rate and unimproved count
            #Warmp training first before checking learning rate because usually accuracy is at 100% at initial iteration
            if self.enable_earlystopping and self.early_stopping_count > 0 and self.iterationCount >= 100:
                #print(f"accuracy: {accuracy}, best_acc: {best_accuracy}, min_acc: {self.min_acc}")
                #print(f"total: {self.min_acc + best_accuracy}")
                if accuracy > best_accuracy + self.min_acc:
                    best_accuracy = accuracy
                    unimproved_count = 0
                else:
                    unimproved_count += 1 #add counter if no improvement in this iteration count
                    if unimproved_count >= self.early_stopping_count:
                        print(f"early_stopping_count: {self.early_stopping_count}")
                        print(f"Early stopping due to no accuracy improvement at iteration {self.iterationCount}")
                        break
            ...
            if self.iterationCount%self.movingAvgCount==0:
                ...
                #Write rules in population log file
                for rule_id, rule in enumerate(self.population.popSet):
                    condition_str = condition_to_symbolic(rule.condition, feature_mins, feature_maxs)
                    predicted_class = int(rule.phenotype) if rule.phenotype is not None else -1

                    self.log_popfile_name.write("{:d} {:d} \"{}\" {:} {:.4f} {:.4f} {:d} {:d}\n".format(
                        self.iterationCount,
                        rule_id,
                        condition_str,
                        predicted_class,
...
def condition_to_symbolic(condition, mins, maxs, tolerance=0.1):
        symbolic = []
        for i, (low, high) in enumerate(condition):
            min_val = mins[i]
            max_val = maxs[i]
            full_range = max_val - min_val
            cond_range = high - low

            # If the condition covers a large portion of the feature range, use #
            if cond_range >= (1 - tolerance) * full_range:
                symbolic.append('#')
            else:
                # You can bin this or just assign '1' to indicate constraint exists
                symbolic.append('1')  # or a custom bin label
